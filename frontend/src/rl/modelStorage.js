...
function exportCsvToFile(records, filename = 'rl_episode_metrics.csv') {
  ...
  const csvBody = [columns.join(',')].concat(
    records.map((record) => columns.map((col) => escapeCell(record[col])).join(','))
  ).join('\n')
