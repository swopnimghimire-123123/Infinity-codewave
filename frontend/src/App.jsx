...
import { clearModel, exportCsvToFile, exportToFile, importFromFile } from './rl/modelStorage.js'
...
function canVehicleProceedFromBackend(signal, vehicle) {
  ...
  // During yellow/all-red, allow only vehicles already at or inside the stop line to clear out.
  if (phaseType !== 'green') return vehicle.distance >= vehicle.path.stopDistance - 2
...
function nextRLSignalState(signal, requestedAction, dt, state, frontDemand = null, liveQueues = null) {
  ...
  const currentPhase = RL_PHASES[Math.max(0, RL_PHASES.indexOf(signal.phase))]
  const pendingPhase = RL_PHASES.includes(signal.pending_phase) ? signal.pending_phase : currentPhase

  function toRLSwitchState(targetPhase) {
    if (!targetPhase || targetPhase === currentPhase) {
      return { phase: currentPhase, phase_type: 'green', elapsed: 0, pending_phase: null }
    }
    return { phase: currentPhase, phase_type: 'yellow', elapsed: 0, pending_phase: targetPhase }
  }

  if (signal.phase_type === 'all_red') {
    return { phase: pendingPhase, phase_type: 'green', elapsed: 0, pending_phase: null }
  }

  if (signal.phase_type === 'yellow') {
    if (elapsed < RL_YELLOW_DURATION) {
      return { phase: currentPhase, phase_type: 'yellow', elapsed, pending_phase: pendingPhase }
    }
    return { phase: pendingPhase, phase_type: 'green', elapsed: 0, pending_phase: null }
  }
  ...
  if (inCongestionOverride) {
    ...
    if (currentPhase !== emergencyPhase) {
      return toRLSwitchState(emergencyPhase)
    }
    return { phase: currentPhase, phase_type: 'green', elapsed, pending_phase: null }
  }

  // If current green serves nobody near the stop line, redirect quickly.
  if (elapsed >= 2 && currentFrontDemand === 0 && highestFrontDemand > 0) {
    return toRLSwitchState(highestFrontPhase)
  }
  ...
  if (action === 1) {
    ...
    if (!forceSwitch && activePressure > targetPressure + 0.03) {
      ...
      return toRLSwitchState(targetPhase)
    }
  }
...
function App() {
  ...
  const simRef = useRef({
    vehicles: [],
    signal: { phase: 'NS_straight', phase_type: 'green', elapsed: 0, lights: null, pending_phase: null },
  ...
  async function uploadMetricsToBackend(metrics) {
    if (!metrics) return
    try {
      const r = await fetch('http://127.0.0.1:8000/rl/metrics', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ runInfo: result.runInfo, episodeMetrics: metrics }),
      })
      if (!r.ok) throw new Error('upload failed')
      alert('Metrics uploaded to backend successfully.')
    } catch (err) {
      alert(`Metrics upload failed: ${err.message}`)
    }
  }
...
if (signalFetchRef.current >= SIGNAL_REAL_INTERVAL) {
  ...
  const fallback = nextSignalLocal(sim.signal, backendStrategy, queues, approaching, signalDt)
          ...
          if (failureActive) {
            ...
            sim.signal.pending_phase = next.pending_phase ?? null
            ...
            <div className="rl-panel">
              ...
              {trainingState === 'done' && (
                <>
                  ...
                  <button type="button" onClick={() => exportToFile({ runInfo: result.runInfo, episodeMetrics: result.episodeMetrics }, 'rl_episode_metrics.json')}>
                    Download Metrics JSON
                  </button>
                  <button type="button" onClick={() => exportCsvToFile(result.episodeMetrics, 'rl_episode_metrics.csv')}>
                    Download Metrics CSV
                  </button>
                  <button type="button" onClick={() => uploadMetricsToBackend(result.episodeMetrics)}>
