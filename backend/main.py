...
class RLMetricsPayload(BaseModel):
    runInfo: Optional[dict] = None
    episodeMetrics: Optional[List[dict]] = None
...
@app.post("/rl/metrics")
def save_rl_metrics(metrics: RLMetricsPayload):
    """Save RL episode metrics exported from the frontend training worker."""
    logs_dir = os.path.join(os.path.dirname(__file__), 'logs')
    os.makedirs(logs_dir, exist_ok=True)
    target_path = os.path.join(logs_dir, f'rl_metrics_{int(time.time())}.json')
    with open(target_path, 'w', encoding='utf-8') as f:
        json.dump(metrics.dict(exclude_none=True), f, indent=2)
